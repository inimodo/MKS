...
mks::functions::encrypt(BRANCH * b_pBranch)
{
	...
	PlotMessage((char *)"Encrypting ", 11);
	PlotMessage(
	...
	for (INT16 i_Fileindex = 0; i_Fileindex < mst_Openfiles.GetFilestack()->fs_Filestack.i_Files; i_Fileindex++)
	{
		...
		i_Status = encryption::EncryptStream(&(mst_Openfiles.GetFilestack()->fs_Filestack.fp_pStack[i_Fileindex]), i_pKey, i_Keylength,&i_Offset);
