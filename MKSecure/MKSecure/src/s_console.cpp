...
mks::console::FuncHeader( BRANCH * b_pBranch)
{
	...
	{
		a_Assets_Buffer[M_BUFFER_FEEDBACK].Tab(1);
		a_Assets_Buffer[M_BUFFER_FEEDBACK].WriteOutput(b_pBranch->a_Args[i_Index].c_pStr, M_COLOR_SYMPOL_UPLIGHT, b_pBranch->a_Args[i_Index].s_Length);
	}
	a_Assets_Buffer[M_BUFFER_FEEDBACK].Break();
	a_Assets_Buffer[M_BUFFER_FEEDBACK].WriteBuffer();
}
...
void __CC
mks::console::PlotHeader(BRANCH * b_pBranch)
{
	a_Assets_Buffer[M_BUFFER_FEEDBACK].WriteChar(195, M_COLOR_INDEXES);
}


void __CC
mks::console::PlotBreak() 
{
	a_Assets_Buffer[M_BUFFER_FEEDBACK].Break();
}

void __CC
mks::console::PlotMessage(DWORD i_Value)
{
	mks::console::FileSize(i_Value, M_COLOR_SYMPOL, M_BUFFER_FEEDBACK);
	a_Assets_Buffer[M_BUFFER_FEEDBACK].WriteBuffer();
}

void __CC
mks::console::PlotMessage(INT16 i_Value, INT16 i_Digits)
{
...
void __CC
mks::console::PlotMessage(char * c_pMessage, INT16 i_Length)
{
	if (c_pMessage != NULL) 
	{
...
{
	CSTR c_WinTitle = CSTR();
	p_WorkDirectory.Build(&c_SoftwareDir);
	c_WinTitle.Set(MAX_PATH);
	c_WinTitle.Append(&c_WinUsername);
	c_WinTitle.Append((char*)" @ ");
	c_WinTitle.Append(&c_SoftwareDir);
	SetConsoleTitle(c_WinTitle.c_pStr);
	...
	if (mst_Openfiles.i_Files != 0) 
	{
		...
		a_Assets_Buffer[M_BUFFER_STATUS].Tab(1);
		a_Assets_Buffer[M_BUFFER_STATUS].WriteOutput(mst_Openfiles.GetFilestack()->fp_Filepath.c_Filename, M_COLOR_FUNCTION, mst_Openfiles.GetFilestack()->fp_Filepath.c_Filename.s_Length);
		a_Assets_Buffer[M_BUFFER_STATUS].Tab(1);
		a_Assets_Buffer[M_BUFFER_STATUS].WriteChar('<', M_COLOR_SYMPOL);
